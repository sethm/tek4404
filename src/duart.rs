...
impl IoDevice for Duart {
    ...
    fn read_8(&mut self, _bus: &mut Bus, address: usize) -> Result<u8, BusError> {
        match address {
            ...
            CSRA => {
                info!("DUART(READ): CSRA: val={:02x}", self.ports[PORT_0].stat);
            ...
            CSRB => {
                info!("DUART(READ): CSRB: val={:02x}", self.ports[PORT_1].stat);
    ...
    fn write_8(self: &mut Self, _bus: &mut Bus, address: usize, value: u8) -> Result<(), BusError> {
        match address {
            ...
            CRA => {
                self.handle_command(value, PORT_0);
            ...
            CRB => {
                self.handle_command(value, PORT_1);
            ...
            _ => {
                info!("DUART(WRITE): UNHANDLED: addr={:08x} val={:02x}", address, value);
            }
        }
